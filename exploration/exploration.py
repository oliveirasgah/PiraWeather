...
for year in range(firstYear, currYear + 1):
    ...
    while True:
        ...
        columns_data = [
            "Chuva_mm" if column == "Chuva" or column == "Precip" else column
            for column in columns_data
        ]
        columns_data = [
            columns_data[i] + str(i)
            if columns_data[i] is not np.nan and columns_data.count(columns_data[i]) > 1
            else columns_data[i]
...
df_met[["Dia", "Horas"]] = pd.DataFrame(
    df_met.apply(centralize_columns_info, axis=1).to_list(), columns=["Dia", "Horas"]
)
...
df_met["Horas"] = df_met["Horas"].apply(
    lambda x: x - (int(x / 100) * 40) if not pd.isna(x) else x
)
...
def create_timestamp(row):
    ...
    if not (np.isnan(row["Dia"]) or np.isnan(row["Horas"])):
        ret = dt(row["yearRef"], 1, 1) + td(days=row["Dia"] - 1, minutes=row["Horas"])

    return ret


df_met["TIMESTAMP"] = df_met.apply(create_timestamp, axis=1)
...
# %%
df_met[df_met["yearRef"] == 1997]

# %%
df_met["TIMESTAMP"] = pd.to_datetime(df_met["TIMESTAMP"])

# %%
df_met["Date"] = df_met["TIMESTAMP"].dt.date

# %%
df_met_daily = df_met.groupby("Date").agg(
    {
        "Tar_AVG": ["min", "mean", "max"],
        "Chuva_mm": "sum",
        "UR_inst": "mean",
        "Vvento_ms_AVG": ["min", "mean", "max"],
    }
)
df_met_daily.columns

# %%
columns_agg = []

for tup in df_met_daily.columns:
    columns_agg.append(tup[0] + "_" + tup[1])

print(columns_agg)

# %%
df_met_daily.columns = columns_agg

# %%
df_met_daily.reset_index()
